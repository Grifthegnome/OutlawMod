...
namespace EvilBelow
{
    public static class EBSpawnEvaluator
    {
        ...
        const string SPAWN_EXCLUSION_POI_TYPE = "ebSpawnBlocker";
        ...
        public static bool CanSpawnEBCreature( Vec3d position, AssetLocation code )
        {
        ...
        private static bool SpawnExcludedBySafeZone(Vec3d position, AssetLocation code)
        {
            ...
            double safeZoneDaysLeft = Math.Max(EBGlobalConstants.startingSpawnSafeZoneLifetimeInDays - totalDays, 0);
            double safeZoneRadius = EBGlobalConstants.startingSpawnSafeZoneRadius;

            //If our starting safety zone has a lifetime (any negative value means never despawn).
            if (EBGlobalConstants.startingSafeZoneHasLifetime)
            {
                //If we are cofigured to shrink the safe zone over time. Figure out how big the spawn zone should be on this calender day.
                if (EBGlobalConstants.startingSafeZoneShrinksOverlifetime)
                {
                    safeZoneRadius = MathUtility.GraphClampedValue(EBGlobalConstants.startingSpawnSafeZoneLifetimeInDays, 0, EBGlobalConstants.startingSpawnSafeZoneRadius, 0, safeZoneDaysLeft);
                }
            ...
            if (!EBGlobalConstants.startingSafeZoneHasLifetime || (safeZoneDaysLeft > 0))
            {
        ...
        */
        ...
        private static bool SpawnExcludedByLandClaim(Vec3d position, AssetLocation code)
        {
            ...
            if (EBGlobalConstants.claimedLandBlocksOutlawSpawns)
            {
